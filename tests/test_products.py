...
def test_create_product():
    ...
    data = response.json()
    assert data["name"] == "Test Product"  # Verify the product name
    assert data["stock"] == 100  # Verify the stock quantity
...
def test_get_product_by_id():
    ...
    assert create_response.status_code == 200  # Ensure the product was created
    product_id = create_response.json()["id"]  # Extract the product ID

    # Step 2: Retrieve the product by its ID
    get_response = client.get(f"/api/products/{product_id}/")
    assert get_response.status_code == 200  # Ensure the request was successful
    assert get_response.json()["id"] == product_id  # Verify the product ID matches
